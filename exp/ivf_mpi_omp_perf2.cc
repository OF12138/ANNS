...
//   1. Phase breakdown        — coarse / fine(OMP) wall times per query
//   2. MPI communication      — one MPI_Gather at batch end, amortized/query
//   3. Per-OMP-thread HW counters (rank 0, representative):
//        cycles, instructions, IPC
//        L1-dcache misses + accesses  → L1 miss rate %
//        LLC misses + accesses        → LLC miss rate %,  LLC MPKI
//        branch misses + instructions → branch miss rate %
//        cycles/vec
//   4. Intra-process OMP load balance — vecs per thread (rank 0)
//   5. Cross-process load balance     — vecs + fine-time (MPI_Reduce)
//
// Counter layout per OMP thread (7 FDs):
//   fd_cyc  PERF_COUNT_HW_CPU_CYCLES
//   fd_ins  PERF_COUNT_HW_INSTRUCTIONS
//   fd_l1m  L1-dcache read misses
//   fd_l1a  L1-dcache read accesses
//   fd_llcm LLC read misses
//   fd_llca LLC read accesses
//   fd_brm  branch mispredictions
//   fd_bri  branch instructions
//
// Each counter that fails to open is treated as "unavailable" and its
// dependent metrics are suppressed — no hard failure.
//
...
#define PROFILE_QUERIES  200    // P × (WARMUP+PROFILE) ≤ 2000 for DEEP100K
...
// =============================================================================
// Per-thread counter bundle
// All fields are accumulated over PROFILE_QUERIES queries.
// =============================================================================
struct ThreadCounters {
    uint64_t vecs = 0;
    uint64_t cyc  = 0;
    uint64_t ins  = 0;
    uint64_t l1m  = 0;   // L1-dcache read misses
    uint64_t l1a  = 0;   // L1-dcache read accesses
    uint64_t llcm = 0;   // LLC read misses
    uint64_t llca = 0;   // LLC read accesses
    uint64_t brm  = 0;   // branch mispredictions
    uint64_t bri  = 0;   // branch instructions
    bool     ok   = false;
};
...
// =============================================================================
// Helpers to print cache / branch rows
// =============================================================================
...
int main(int argc, char** argv)
{
    ...
    if (rank == 0) {
        std::cerr << "════════════════════════════════════════════════════\n";
        std::cerr << " IVF_MPI_OMP_PLAN2 — detailed perf profiling\n";
    ...
    if (!loaded) {
        if (rank == 0) { ivf_build(idx, base, base_number, vecdim, IVF_NLIST, 25, false); ivf_save(idx, cache_path); }
        MPI_Barrier(MPI_COMM_WORLD);
        if (rank != 0) ivf_load(idx, cache_path);
    }
    if (rank == 0) std::cerr << "[index] " << (loaded ? "loaded" : "built") << "\n";

    // ── Probe counter availability (non-fatal) ────────────────────────────────
    // Open one test FD of each type to see what the kernel supports.
    ...
    // ── Accumulators — rank 0's OMP threads ──────────────────────────────────
    std::vector<ThreadCounters> thread_acc(nthreads);

    // ── Per-process scalars ───────────────────────────────────────────────────
    double t_coarse_s=0, t_fine_s=0;
    double proc_vecs=0;
    ...
    // Scratch per-query per-thread counters (rank 0 only)
    ...
    for (int local_i = 0; local_i < TOTAL_PER_PROC; ++local_i)
    {
        ...
        #pragma omp parallel num_threads(nthreads)
        {
            ...
            // Open all 8 FDs from inside this thread (pid=0 = this thread)
    ...
    // ── Final MPI_Gather (one per batch) ─────────────────────────────────────
    ...
    if (rank == 0) {
        ...
        std::cout << "\n════════════════════════════════════════════════════\n";
        std::cout << " IVF_MPI_OMP_PLAN2 — detailed perf profile\n";
        ...
        std::cout << "  fine   (OMP " << nthreads << " threads) : " << t_fine_s/N*us << " us\n";
        ...
        if (have_perf) {
            ...
            double ipc_tot = (tc>0) ? ti/tc : 0.0;
            double cpv_tot = (tv>0) ? tc/tv : 0.0;
            std::cout << std::setw(6)  << "all"
                      << std::setw(8)  << std::setprecision(1) << tv
                      << std::setw(7)  << std::setprecision(2) << ipc_tot
                      << std::setw(9)  << std::setprecision(2) << cpv_tot;
            if (have_l1) {
            ...
            if (have_llc) {
                ...
                double mpki = (ti>0) ? (tot_llcm/N)/(ti/1000.0) : 0.0;
                if (r>=0) std::cout << std::setw(9) << std::setprecision(1) << r << "%"
                                    << std::setw(9) << std::setprecision(2) << mpki;
                else      std::cout << std::setw(10) << "n/a" << std::setw(9) << "n/a";
            }
            ...
            if (have_l1) {
                double r = rate(tot_l1m, tot_l1a);
                double mpki_l1 = (tot_ins>0) ? (tot_l1m/N)/((tot_ins/N)/1000.0) : 0.0;
                std::cout << "  L1-dcache miss rate : ";
                if (r>=0) std::cout << std::setprecision(2) << r << "%"
                                    << "  (" << std::setprecision(1) << tot_l1m/N
                                    << " misses / " << tot_l1a/N << " accesses/query)"
                                    << "  MPKI=" << std::setprecision(2) << mpki_l1 << "\n";
                else      std::cout << "n/a\n";
            }
            if (have_llc) {
                double r = rate(tot_llcm, tot_llca);
                double mpki_llc = (tot_ins>0) ? (tot_llcm/N)/((tot_ins/N)/1000.0) : 0.0;
                std::cout << "  LLC miss rate       : ";
                if (r>=0) std::cout << std::setprecision(2) << r << "%"
                                    << "  (" << std::setprecision(1) << tot_llcm/N
                                    << " misses / " << tot_llca/N << " accesses/query)"
                                    << "  MPKI=" << std::setprecision(2) << mpki_llc << "\n";
                else      std::cout << "n/a\n";
            }
            if (have_br) {
                double r = rate(tot_brm, tot_bri);
                ...
                if (r>=0) std::cout << std::setprecision(2) << r << "%"
                                    << "  (" << std::setprecision(1) << tot_brm/N
                                    << " mispred / " << tot_bri/N << " branches/query)\n";
                else      std::cout << "n/a\n";
            }
            std::cout << "\n";
            ...
            // Fallback: no perf — vec counts + wall-time only
        ...
        std::cout << "  [Query assignment is static: imbalance reflects query\n"
                     "   difficulty variance, not structural partition skew.]\n";
