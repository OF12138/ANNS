// =============================================================================
// main_hnsw_hnsw_mpi_omp.cc -- HNSW on HNSW, MPI (nprobe-split) x OMP (multi-entry)
//
// Two-level HNSW:
//   Coarse: HNSW built on IVF centroids (replaces flat centroid scan).
//   Fine:   per-cluster HNSW beam search (same as IVF+HNSW).
//
// MPI: P processes split the nprobe selected clusters.
// OMP: T threads per process run multi-entry beam search within each cluster.
//
// Compile:
...
#define COARSE_M              16
#define COARSE_EF_CONSTRUCTION 200
#define COARSE_EF_SEARCH      32    // >= NPROBE; extra exploration in coarse HNSW
...
#include "ARM/Alg_parallel/hnsw_hnsw_mpi_omp.h"
...
int main(int argc, char* argv[])
{
    ...
    if (rank == 0) {
        std::cerr << "========================================\n";
        std::cerr << "[config] HNSW-on-HNSW  MPI(nprobe-split) x OMP(multi-entry)\n";
        std::cerr << "[config] mpi_procs="        << size
                  << "  omp_threads="             << nthreads
                  << "  total_workers="           << size * nthreads << "\n";
        std::cerr << "[config] ivf_nlist="        << IVF_NLIST
                  << "  ivf_nprobe="              << IVF_NPROBE       << "\n";
        std::cerr << "[config] fine_M="           << HNSW_M
                  << "  fine_ef_construction="    << HNSW_EF_CONSTRUCTION
                  << "  fine_ef_search="          << HNSW_EF_SEARCH   << "\n";
        std::cerr << "[config] coarse_M="         << COARSE_M
                  << "  coarse_ef_construction="  << COARSE_EF_CONSTRUCTION
                  << "  coarse_ef_search="        << COARSE_EF_SEARCH << "\n";
        std::cerr << "========================================\n";
    }

    // ── Load query + ground truth (all processes) ─────────────────────────────
    ...
    if (loaded) {
        ...
        // Cache miss: only rank 0 builds and saves to avoid OOM on all processes
        if (rank == 0) {
            ...
            std::cerr << "[data] base=" << base_number
                      << "  queries=" << test_number
                      << "  dim=" << vecdim << "\n";
            std::cerr << "[build] building HNSW-on-HNSW...\n";

            hnsw_hnsw_build(idx, base, base_number, vecdim,
                            IVF_NLIST, 25,
                            HNSW_M, HNSW_EF_CONSTRUCTION,
                            COARSE_M, COARSE_EF_CONSTRUCTION);
            delete[] base;
            ...
            hnsw_hnsw_save(idx, cache_dir);
            hnsw_hnsw_free(idx);
            std::cerr << "[build] saved.\n";
        }

        // All ranks synchronise; non-zero ranks then load from cache
        MPI_Barrier(MPI_COMM_WORLD);

        gettimeofday(&tb0, NULL);
        hnsw_hnsw_load(idx, cache_dir,
                       HNSW_M, HNSW_EF_CONSTRUCTION,
                       COARSE_M, COARSE_EF_CONSTRUCTION);
        gettimeofday(&tb1, NULL);
        if (rank == 0)
            std::cerr << "[build] all processes loaded from cache: "
                      << tv_diff_us(tb0, tb1) / 1000 << " ms\n";
    }
    ...
    for (size_t i = 0; i < test_number; ++i) {
        hnsw_hnsw_mpi_omp_search_query(
            ...
            k, IVF_NPROBE, COARSE_EF_SEARCH, HNSW_EF_SEARCH,
            rank, size, nthreads);
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    for (size_t i = 0; i < test_number; ++i) {
        ...
        auto res = hnsw_hnsw_mpi_omp_search_query(
            idx, q, k, IVF_NPROBE, COARSE_EF_SEARCH, HNSW_EF_SEARCH,
    ...
    double my_coarse_s = tm.t_coarse_s;
    double my_fine_s   = tm.t_fine_s;
    ...
    MPI_Reduce(&my_coarse_s, &coarse_max, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&my_coarse_s, &coarse_min, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
    MPI_Reduce(&my_coarse_s, &coarse_sum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&my_fine_s,   &fine_max,   1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&my_fine_s,   &fine_min,   1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
    MPI_Reduce(&my_fine_s,   &fine_sum,   1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        std::cerr << "  coarse (HNSW nav) : " << coarse_sum / size / N * us << " us  avg\n";
        std::cerr << "  fine   (HNSW srch): " << fine_sum   / size / N * us << " us  avg"
